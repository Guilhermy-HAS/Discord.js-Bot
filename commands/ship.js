...
module.exports.run = async (bot, message, args) => {//exportando o comando como ship
  ...
  if(!membro1 || !membro2) return message.channel.send('Mention two persons to ship please!')
    if(membro1 === membro2) return message.channel.send('Mention two different persons')
  ...
  let desc;
    if(amor > 90) {
      desc = (":sparkling_heart: The love is kind? :sparkling_heart:\n`"+membro1.user.username+"`\n`"+membro2.user.username+"`\n:heart: `"+nomeship+"` This is the perfect couple! :heart:");
    } else if(amor >= 70) {
      desc = (":sparkling_heart: The love is kind? :sparkling_heart:\n`"+membro1.user.username+"`\n`"+membro2.user.username+"`\n:neutral_face: `"+nomeship+"` The love is in the air :neutral_face:");
    } else if(amor >= 45) {
      desc = (":sparkling_heart: The love is kind? :sparkling_heart:\n`"+membro1.user.username+"``\n`"+membro2.user.username+"`\n:no_mouth: `"+nomeship+"` if "+membro2.user.username+" wanted :no_mouth:");
    } else {
      desc = (":sparkling_heart: The love is kind? :sparkling_heart:\n`"+membro1.user.username+"`\n`"+membro2.user.username+"`\n:cry: `"+nomeship+"`Impossible :cry: ");
    }
